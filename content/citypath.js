(function () {
    ...
    if ($tmp == "inn" || $tmp == "rest" || $tmp == "work" || $tmp == "fight") {
        ...
        if ($0 == 7) { $3 = evnt("4:15", "0:100") } else if ($0 == 4 || $0 == 5) { $3 = evnt("1:15", "2:15", "4:15", "3:100") } else {
            ...
            if ($3 == 4) {
                $3 = (rng(4) + 1); if ($hpm > 14) { $4 = `\${btn('fight',"combat([2,1,10,0,${$3},'Bandit'];$cbt='fight.citypath')")}` } else { $tmp = 'fight'; $n = `\${btn('surrender',"$money=0;next('citypath')")}` } if ($epm > 19 && $ep > 0) { $n += ` | \${btn('escape',"$cbt='run';next('citypath')")}` }
