...
<br><br>Money: ${slist('mon',"1:1,10:10,100:100")} ${btn('sub',"$money-=Number(ELM('mon').value)")} | ${btn('add',"$money+=Number(ELM('mon').value)")}
<br><br>Damage: ${btn("health", "$hp-=Math.floor($hpm/10);sbu('hp')")} | ${btn("energy", "$ep-=Math.floor($epm/10);sbu('ep')")} | ${btn("elemental qi", "$qi-=Math.floor($qim/10);sbu('qi')")} | ${btn("recover", "$ep=$epm;$hp=$hpm;$qi=$qim;sbu('hp','ep','qi')")}
