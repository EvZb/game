(function () {//tmpan is reserved | tmp 0-2 should be unlimited events
    switch(tmp){
        ...
        case '3'://find material
            if(tmpn==0){tmpa=[['Sun Crystals.1.6.0.1.7','Rock Flowers.1.5.0.3.7','Spirit Berries.1.7.4.2.7'][rng(2)]]
        ...
        case '4'://find item/chest
            switch(tmpn){
                case 0:tmpn=rng(4);tmpa=[['Purified  Iron '+['Spear','Sword','Knife','Staff','Axe'][rng(4)],'Spirit Potion','Purified Key','Light Robe','Purified Chest'][tmpn]]
                    if(tmpn==0){tmpa[1]=[tmpa[0],1,0,7,tmpa[0].split(' ')[2]+'!0!1!1']}else{tmpa[1]=[[tmpa[0],1,5,6,'0!0!20'],[tmpa[0],1,3,8,x],[tmpa[0],1,1,6,'0!0!1!1']][tmpn-1]}
                ...
                case 2:let $1=data(0);let $2=$1[1].split('.');$2[pos[2]]-=1;$1[1]=$2.join('.');data(0,$1)
                    if(tmpa[0]=='Purified Chest'){remitem('itm','Purified Key');t=`You unlock the chest; it holds a book from a series called "Studying Dragon Fire".`;add('item',['Dragon Fire Book',1,7,6,'1'])}
        ...
        /* GREAT DRAGON?
            switch(tmpa[0]){
                ...
                case 'fall':hp[0]-=10;sbu('hp')
                    if(hp[0]<1){t=`You lose consciousness.`;n=`\${btn("next","tmpa=['faint',w+'terrain/mountains','5',0,'wake','${tmpan.join()}'];tmpn=2;next('other/rest')")}`}
                ...
                case 'fight':if(tmpan[1]==1){tmpan[0]=100-tmpan[0]}
                    ...
                    tmpa=['material','Giant Worm teeth.2.5.4.1.5','0.8.4.0.0.4.4.4.20.18.14','Giant Sand Worm','big bite',x,'world/terrain/mountains','cbt','0','x',tmpan.join()]
