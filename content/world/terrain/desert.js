(function () {//tmpan is reserved. 0 should be reserved for finding nothing. 1 is for monsters
    switch(tmp){
        case x:
        ...
        case '1'://monster hunting/attack
            ...
            $2+=$1[2].split(',')[2].split('.').slice(6).join()
            switch(rng(2)){
        ...
        case '2'://camp/traders
            if(tmpn==0){t=`You encounter a group of hunters. They will give you a good price if you have something they are looking for.`;n=`\${btn('trade',"tmpn=1;next()")} | \${btn('leave',"tmp=x;next()")}`}
                ...
                let $2=[6,7,6,6,6,5,5,5,7,7,7,4];$1.forEach(($a,$b)=>{let $3=mat.indexOf($a);let $4=(11-$2[$b])*5;if($3!=-1){$0+=','+$a+' ('+$4+'):'+$3+'!'+$4}})
        ...
        case '4'://find item/chest
            switch(tmpn){
                ...
                case 1:t=`It's an ${tmpa[0]}.`
                    if(tmpa[0]=='Eroded Chest'&&itm.includes('Eroded Key')){t+=` You can open it with your Eroded Key.`;n=`\${btn('unlock',"tmpn=2;next()")} | `}
        ...
        case '5'://dangerous event (add combat and faint properly)
            switch(tmpa[0]){
                case x:tmpa=['worm','pit'][rng(2)]
                ...
                case 'pit':
                    ...
                    else{tmpn=0;t=`You hide behind some rocks, but the ground below you starts falling into a pit. You try to escape, but fall in as well.`;tmpa=['fall'];`\${btn('fall',"next()")}`};break
                ...
                case 'fight':if(tmpan[1]==1){tmpan[0]=100-tmpan[0]}
                    ...
                    if(tmpn==1){tmp='alert'}else{tmp='stealth'};tmpr(tmp,0,'material,Giant Worm teeth.2.5.4.1.5,8.4.0.4.4.4.20.18.14,Giant Sand Worm,big bite,none,'+tmpan[1]+',tmpa,'+tmpan[0]+',world/terrain/desert','5,1,20,18,14');break
