(function () {//tmpan is reserved | tmp 0-2 should be unlimited events | LOOT/MONSTERS UNFINISHED (Template)
    switch(tmp){
        case x:
        ...
        case '0':
            t=`You find ${['nothing','some old monster tracks in the snow','an empty basket','claw marks on some ice'][rng(3)]}.`
            n=`\${btn('next',"tmp=x;next()")}`;break
        case '1':
            let $1=monster(['Ooze','Winder','Spider','Wolf','Bear','Wraith'][rng(5)],rng(2,1),[x,'Ice','Water','Mist','White'][rng(4)])
            tmpa=[$1[3],$1[4],$1[1],$1[0],$1[5],x,'world/terrain/snow','cbt','0',x,tmpan.join()]
        ...
        case '3'://find material
            if(tmpn==0){tmpa=[['Frozen Mushrooms.1.6.4.2.7','Long Icicles.1.6.5.1.7','Enchanted Snow.1.5.5.3.7'][rng(2)]]
        ...
        /*
            switch(tmpa[0]){
                ...
                case 'fall':hp[0]-=10;sbu('hp')
                    if(hp[0]<1){t=`You lose consciousness.`;n=`\${btn("next","tmpa=['faint',w+'terrain/snow','5',0,'wake','${tmpan.join()}'];tmpn=2;next('other/rest')")}`}
                ...
                case 'fight':if(tmpan[1]==1){tmpan[0]=100-tmpan[0]}
                    ...
                    tmpa=['material','Giant Worm teeth.2.5.4.1.5','0.8.4.0.0.4.4.4.20.18.14','Giant Sand Worm','big bite',x,'world/terrain/snow','cbt','0','x',tmpan.join()]
