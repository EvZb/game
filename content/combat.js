(function () {
    ...
    if (Array.isArray($cbt)) {function $F3($a,$b){let $0=$a[0];let $7=$a[3]-$lvl[3];if($7<0){$0=Math.floor($0/(-$7+1))}else{$0=$0*($7+1)};$0-=$def;if($0>0){$hp-=$0;$t+=` ${$6} #${$b+1} attacks for `+$0+` damage.`}else{$t+=` ${$6} #${$b+1} is too weak to hurt you.`}};$3.forEach($F3);$ep-=1
        let $a = Number($cbt[0]); let $b = Number($cbt[1]); $cbt = $5; if ($a > 0) { $qi -= $a; exp() } else {$a=$atk};let $7=$lvl[3]-$3[$b][3];if($7<0){$a=Math.floor($a/(-$7+1))}else{$a=$a*($7+1)};$a -= $3[$b][1]; if ($a < 0) { $a = 0 }; $3[$b][2] -= $a
