(function () {
    ...
    if (($ep > 0 || $tmp < 0) && $qi > 4) {
        ...
        if ($tmp == 0 || $tmp == -1) {
            $n = `${btn("hunt again", "next('ghunt')")}`; if ($tmp == 0) { $tmp = "none"; $money += 3; $t = `You successfully trap the ghost. <font color="yellow">The client will pay you</font> after you exercise it.` }
            if ($tmp == -1) { $tmp = "none"; $qi -= 5; $t = `You <font color="yellow">incorrectly</font> guessed the ghost. The trap failed, wasting your qi.` }; sbu('qi')}
