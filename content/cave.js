(function () {
    ...
    if ($tmp == "book") { $1 = ` | ${btn("take book", "$tmp='take';tec('add','Basic Cultivation--$qi-=10;exp();time([0,0,4,0])--basic cultivation methods');next('cave')")}`; $t = `You pickup a book from the ground. "${col("orange", "Basic Cultivation")}"` } else if ($tmp == "take") {
        $t = `You take the book.`
