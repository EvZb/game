(function () {
    ...
    else if($0.length>0){$2+=`<br><br>\${btn('use',"iuse(elm('items').value);next('menu/items',0)")} \${lst('items',"${$0.join(',')}")} \${btn('destroy',"rem('item',elm('items').value);next('menu/items',0)")}`}
    ...
    if($4.length>0){$2+=`<br><br>\${lst('mats',"${$4.join(',')}")} \${btn('destroy',"rem('material',elm('mats').value);next('menu/items',0)")}`}
