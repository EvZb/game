(function () {//tmpn=0 to 100 (distance) | tmp | tmpa=[in/out, terrain, major/terrain, terrain/major, turn/none]
    ...
    case 'explore'://exploration option
        ...
        if((tmpa[0]=='out'&&$4[0][0]==pos[2]&&$4[0][1]*10==tmpn)||(tmpa[0]=='in'&&$4[0][0]==(pos[2]+4)%8&&(10-$4[0][1])*10==tmpn)){
            ...
            n=`\${btn('next',"${$5}ptime([30]);tmpan=[tmpn,0,0];if(tmpa[0]=='in'){tmpan[1]=1};tmp=x;tmpn=0;next(w+'minor/${chunk[loc()].arr(2)}')")}<br><br>\${btn('back',"tmp=x;next()")}`}
        else{$5=rng(5)
    ...
    case 'event'://random travel events
        switch(rng(2)){
            case 0:t+=`you find an injured person along the path. They seem to have been running and is too exhausted to move.`;break
            case 1:t+=`someone walks up to you before threatening you into handing over your money.`;break
            case 2:t+=`a strange person walks up to you and asks you to give him some money in exchange for an item.`;break}
        if(hp[0]<1){t+=` You lose consciousness.`;n=`\${btn("next","tmpa=['faint','other/explore',x,${tmpn},x,'0'];tmpn=2;next('other/rest')")}`}else{n=`${btn("next","tmp=x;next()")}`};break
    }
    end()
})()