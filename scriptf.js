...
//use functions
function worldgen($a=rng(4)) {world.push(window['namegen'+ngen]().replace(' ','').slice(0,9));wgen.push(Array.from(Array(100)).map(()=>weight[rng(31)]).join(''));wgen2.push(Array(101).join(' '));wtype=[atype[$a]];wppl.push(x)}//generates a new world $a is the (optional) atype
